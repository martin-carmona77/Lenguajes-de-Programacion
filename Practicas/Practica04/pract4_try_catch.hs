...
data LamAB =  Var String |
              ...
              Menor LamAB LamAB |
              Lambda String LamAB |
              Catch LamAB LamAB |   --corresponde a catch(e₁,e₂)
              Error |               --corresponde a la constante 'error'
...
data Marco = MSumI () LamAB            | --Marco suma izq
             ...
             MCatchI () LamAB          | --Marco catch izq
             MCacthD LamAB ()          | --Marco catch derecha
...
freeVars e = case e of
               ...
               Error -> []
               Catch e1 e2 -> freeVars e1 ++ freeVars e2
...
eval1 e = case e of
            ...
            Ev (p,Error) -> Pr (p,Error)
            Pr (m:p,Error) -> case m of
                                (MCatchI () e2) -> Ev (p,e2)
                                _ -> Pr (p,Error)
...
--Pruebas try-catch
prueba1' = evalK $ Ev ([], Catch (Ifte (Neg (VBool False)) (Suma (VNum 5) (VNum 7)) (VNum 77)) (Error))
prueba2' = evalK $ Ev ([(MSumI () (Suma (VNum 4) (VNum 4))),(MSumI () (VNum 4)),(MCatchI () (Neg (Menor (VNum 3) (VNum 4))))],Error)
prueba3' = evalK $ Ev ([], Catch (Ifte (Neg(Menor (VNum 0)(VNum 1)))(Suma (VNum 4)(VNum 7))(Error)) Error)
prueba4' = evalK $ Ev ([], Ifte (Menor (Suma (VNum 5) (VNum 8))(VNum 14)) (Error)
                          (Let "x" (VNum 3) (Suma (Var "x") (Var "x"))))
